...
GPUI is a retained-mode UI framework built for the Zed code editor. For our engine we did a major rewrite/port to a version built entirely on wgpu. We use it for our own projects — editors, tools, browser-like applications with complex layouts, thousands of interactive elements, and real-time rendering requirements. It works. But when the UI gets complex, it stops working well.

The problem isn't subtle. Open a properties panel with a few hundred settings widgets, the moment we scroll the frame rate drops, the scroll handle lags visibly behind the mouse cursor. The scroll stutters. The CPU profile shows the main thread flat-out for 12-16 milliseconds doing work that, in a browser or a native UI toolkit, would take essentially zero. The work isn't useless — it's sorting primitives by draw order, computing damage rectangles, uploading changed data to GPU buffers, encoding render passes. All necessary work. But none of it should be running on the main thread.
...
The architecture has two major components: a **dedicated compositor thread** that takes over all scene finishing, GPU upload, and command encoding work; and an **overscroll buffer** that makes scrolling a near-zero-cost operation by doing what browsers have done for over a decade — pre-rendering content outside the viewport and shifting pixels on scroll instead of re-rendering everything from scratch.

We'll start with the problem, then walk through each piece of the design, then talk about all the ways it could fail and what we're doing about each one.

---

## What the Main Thread Does Today

Before we can design a fix, we need to understand exactly where the time goes. GPUI's frame pipeline, from winit event to pixels on screen, looks like this:
...
Every step runs on the main thread, in sequence, with no parallelism between any two steps. The earlier steps produce data consumed by the later steps — `draw_roots` paints primitives into the scene, `finish` sorts and indexes them, `present` uploads and draws them — so there's a real dependency chain. But not all dependencies are equal. Some steps are pure computation on already-owned data. Some are GPU API calls that are thread-safe by design. Some are genuinely main-thread-bound by wgpu's API constraints.

If we were to profile this pipeline on a realistic workload — say, a code editor view with tens of thousands of primitives across hundreds of dirty views — we'd expect the per-frame breakdown to reveal a clear split:
...
When the user scrolls far enough that the viewport would intersect un-rendered content — the edge of the overscroll buffer — the browser does one of two things depending on the engine. In Chrome/Blink, it rasterizes new tiles at the page's edge in a background compositor thread, extending the valid area of the texture. In Safari/WebKit, it re-centers the overscroll buffer by copying existing content to a new position within the texture and rasterizing only the newly-exposed edges. Either way, the expensive operation — rasterization — is a partial update that affects only the margins of the buffer, not the entire visible area. The next thousand pixels of scroll are free again.

GPUI as a whole doesn't do any of this. GPUI is a retained-mode framework, meaning elements define their structure once and the framework diffs changes between frames. But "retained" here applies to the element tree, not to the rendered output. Every frame, GPUI walks the element tree, generates scene primitives, sorts them, uploads them, and draws them. If the user scrolls, the scroll offset changes, the element tree re-renders its children at new positions, and the entire pipeline runs again. The content is identical to the previous frame but shifted by a few pixels. The framework pays the full cost of re-rendering all of it.
...
The overscroll buffer approach fixes this by making "content changed" and "viewport position changed" two different concepts. When the user scrolls:
- The scroll offset updates in the application state.
- If the new visible rect lies entirely within the pre-rendered overscroll buffer: no rendering work happens. Zero. The next present picks a different subrect of the overscroll buffer for the blit, and the swapchain shows the scrolled view. The CPU does not touch the element tree, does not sort primitives, does not upload data, does not create bind groups. Total cost: one blit command in a command buffer, submitted and presented.
- If the new visible rect extends beyond the rendered area: the compositor renders only the newly-exposed tiles — the thin strip of content that just became visible at the edge. This is a partial update, not a full re-render. The rest of the overscroll buffer is reused as-is.
- If the user has scrolled so far that the content origin in the overscroll buffer has drifted too far from center: the compositor performs a GPU-side copy to shift existing content within the texture, then renders the newly-exposed edges that were uncovered by the shift.

For a typical scroll interaction — say, reading through a document — the profile looks like:

- **First scroll event:** Full render into overscroll buffer. Normal frame cost on main thread.
- **Subsequent scrolls within buffer:** Zero CPU rendering work. Just a blit.
- **Scroll hits edge of buffer:** Partial tile extension on the compositor thread. Minimal work on main thread.
- **Scrolling continues within new bounds:** Zero CPU rendering work again.
...
## Architecture: The Compositor Thread

The design centers on a dedicated compositor thread that takes ownership of all rendering work between scene building and swapchain presentation. The main thread's role is reduced to:
1. Building the element tree and painting primitives into a `Scene` (draw_roots).
2. Lightweight bookkeeping: focus tracking, hit testing, cursor styles.
3. Packaging the scene and sending it to the compositor.
4. When a completed frame is available: blitting the pipeline texture to the swapchain and presenting.

The compositor thread does everything else: scene finishing (sort, damage, changed ranges), GPU uploads (queue.write_buffer), command encoding, and submission.

### Why a Thread Instead of a Task Pool

We considered using the existing `BackgroundExecutor` (a priority-threadpool with 3 levels) for this work. The appeal is obvious — no new thread management, no new infrastructure. We rejected it for a few reasons.

First, the compositor needs guaranteed access to GPU resources that can't be contended with other background work. If a file I/O task and a compositor task are both scheduled on the threadpool, the compositor could be delayed by a slow disk read. File I/O should never delay rendering. A dedicated thread avoids this — the compositor is always a scheduling decision away from a free core.
...
The cost is one OS thread per compositor instance. For an application with one window, that's one extra thread. For an application with many windows, each window would need its own compositor, which could be many threads. We have a mitigation for this (using the threadpool for multi-window cases), which we'll get to later.
...
We chose `flume` over `crossbeam` or `std::sync::mpsc` because flume is fast (near the theoretical minimum for channel operations) and has a convenient `try_send` / `try_recv` API that we use heavily for non-blocking communication. The channels are bounded with capacity 1:
...
The main thread doesn't need the finished scene back — it was already moved into the compositor. All the main thread needs to know is that the pipeline texture has valid content and what bounds of that content are up-to-date.
...
Its `draw` method (which currently does everything) becomes a thin wrapper:
...
The `present()` call becomes: poll for completed frame, blit, submit, present. That's it. No uploads, no render pass, no bind group management. The heavy work was already done on the compositor thread.

### Pipeline Texture Details
...
We verified this by auditing every field of every type transitively referenced by `Scene`. The full list includes:

- `Quad`, `Shadow`, `BackdropBlur`, `Underline`, `MonochromeSprite`, `PolychromeSprite`, `PaintSurface`, `Path<ScaledPixels>` — all inline data (f32, u32, Bounds, Hsla, ContentMask), no heap references, no Rc/RefCell.
- `SceneSegmentPool` — contains `u64` and `FxHashSet<SceneSegmentId>`.
- `BoundsTree<ScaledPixels>` — contains `Option<usize>` and `Vec<Node>`.
- `ChangedRanges` — eight `Vec<Range<usize>>`.
- `PrimitiveBatches` — `HashMap<PrimitiveTypeId, PrimitiveBatch>` and `Vec<PrimitiveTypeId>`.
- `PaintOperation` — an enum of `Primitive`, `StartLayer(Bounds)`, and `EndLayer`.

No `Rc`, no `RefCell`, no raw pointers, no `NonNull`, no thread-local types. The Scene and everything it owns can be moved to another thread by `mem::take`, processed, and never sent back. (The finished scene replaces `previous_scene` in the compositor state; the main thread never sees it again.)
...
fn upload_path_vertices(&mut self, scene: &Scene) {
    ...
    // Upload to GPU buffer using the same PrimitiveUploadState tracking
...
The upload state is compositor-owned (`PrimitiveUploadStates` in `CompositorState`), so there's no contention with the main thread. The generation counter comes from the scene, which is compositor-owned after the commit.

For the `Full` decision, we call `ensure_buffer_size` which may re-create the GPU buffer. This calls `device.create_buffer()`, which is thread-safe. For the `Ranges` decision, we iterate the dirty byte ranges and call `queue.write_buffer()` for each.
...
The render pass draws all primitive batches into the overscroll buffer. This is the same draw logic that `WgpuRenderer::draw()` currently uses — the same batched draw calls for quads, shadows, backdrop blurs, underlines, monochrome sprites, polychrome sprites, paths, and surfaces. The only difference is the render target: instead of a swapchain texture acquired per-frame, it's a persistent pipeline texture owned by the compositor.
...
The `wgpu::Queue` is behind `Arc` and shared with the main thread. Submitting to it from the compositor thread is safe because `Queue::submit()` uses internal synchronization (a `Mutex` in wgpu-core). The main thread also submits to the same queue (for blit commands). GPU execution order is determined by submission order, and the channel ensures that the compositor's submit happens before the main thread's blit submit.
...
The compositor stores the finished scene as `previous_scene` for the next frame's damage comparison, marks the overscroll buffer's rendered area as valid, and sends a completion signal to the main thread. The main thread picks this up on the next `try_present()` call.

---

## The Overscroll Buffer in Detail

The overscroll buffer is what transforms scrolling from an expensive operation into a cheap one. The idea is simple — render a larger area and shift the viewport — but the implementation has several subtleties worth discussing.

### The Three-Case Scroll Model

Every scroll event is classified into one of three cases:
...
fn schedule_tile_extension(&mut self) {
    ...
    // Compute the intersection of the viewport with the unrendered area
    ...
    // Build a scene that includes only the needed region
    // (This is a full draw_roots — we can't partially build a scene)
    // But the compositor will only render tiles that intersect `needed`
...
fn handle_extend_tiles(&mut self, scene: Scene, render_rect: Bounds<ScaledPixels>) {
    ...
    // Only begin a render pass if we have actual work to do
    ...
    if has_work {
        ...
        // Draw only dirty chunks that intersect render_rect
...
fn handle_recenter(
    ...
    let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
        color_attachments: &[Some(wgpu::RenderPassColorAttachment {
            view: &self.overscroll.view,
            ops: wgpu::Operations {
                load: wgpu::LoadOp::Load,  // Don't clear shifted content
                store: wgpu::StoreOp::Store,
            },
            ..Default::default()
        })],
    ...
    // Only render chunks that intersect the newly-exposed area
    ...
    for chunk in scene.chunks.iter().filter(|c| c.dirty) {
        if let Some(bounds) = scene.chunk_bounds(chunk) {
            if bounds.intersects(new_edges) {
                // Clip scissor to the intersection
...
fn try_present(&mut self) -> PresentResult {
    // Priority 1: Compositor has a completed frame ready
    if self.renderer.try_present_completed() {
        return PresentResult::Presented;
    }
    
    // Priority 2: Overscroll buffer covers the visible viewport
    ...
    // Priority 3: Viewport is close to overscroll edge — schedule extension
    if self.overscroll.nearly_exceeded(self.current_viewport()) {
        self.schedule_tile_extension();
        // Fall through to cached framebuffer for this frame
    }
    
    // Priority 4: No new content and overscroll doesn't cover viewport
    // Present cached persistent framebuffer
    self.renderer.present_framebuffer_only();
    PresentResult::ShowCached
}
```

This is called on every `RedrawRequested`, not just when the state is dirty. The winit event loop runs with `ControlFlow::Poll`, so we get regular callbacks. On each callback, we try to present. Most of the time, a frame is either ready from the compositor or the overscroll buffer covers the viewport, and we present with minimal work.

The `overscroll.covers()` check is a simple bounds comparison:

```rust
fn covers(&self, viewport: Bounds<ScaledPixels>) -> bool {
    let rendered = self.rendered_rect();
    rendered.contains_rect(viewport)
}
```

The `nearly_exceeded()` check adds a margin — we trigger a tile extension when the viewport approaches the rendered area's edge, so the extension has time to complete before the user actually reaches the edge:
...
fn handle_scroll_event(&mut self, delta: ScrollDelta, cx: &mut App) {
    // Always route the event to the focused element for hit testing and focus
    // (the element tree needs to know where its children are)
    self.dispatch_scroll_event(delta, cx);
    
    // Update our scroll tracker
    self.scroll_state.total_offset += delta.pixel_delta();
    
    // Now check: did the element tree produce any non-scroll changes?
    // We use a heuristic: if the only views that called cx.notify() during
    // the event dispatch were scroll containers, and no other state changed,
    // then this was a scroll-only event.
    //
    // The exact heuristic will depend on how we instrument the element tree
    // to distinguish scroll notifications from content notifications.
    // One approach: scroll containers call a different method (cx.scrolled())
    // that doesn't mark the view dirty but still triggers the layout offset
    // update. Non-scroll views still call cx.notify() for content changes.
    
    if self.scroll_state.scroll_only {
        // Don't call cx.notify() or trigger draw_roots
        // Just request a redraw for the next try_present()
        self.needs_present.set(true);
    } else {
        // Content changed — do a full commit
...
The cleanest version of this requires changes to the scroll container elements (`UniformList`, `List`, scrollable `div`). Instead of calling `cx.notify()` on every scroll event, they would call a new method that updates scroll state without triggering a full re-render. The exact API is still being designed, but the intent is:

```rust
// Current (triggers full re-render):
fn on_scroll(&mut self, delta: Vec2<f32>, cx: &mut ViewContext<Self>) {
...
fn on_scroll(&mut self, delta: Vec2<f32>, window: &mut Window, cx: &mut ViewContext<Self>) {
    self.scroll_offset += delta;
    // Record the new scroll offset so hit testing and focus work correctly
    // But don't trigger draw_roots — the compositor will handle this via
...
This is the most architecturally invasive part of the design — it changes the fundamental contract of how scroll containers interact with the rendering pipeline. We've gone back and forth on whether to implement this as a new API method on scroll containers or as an automatic detection heuristic in the window's event handler. The current thinking is to support both: scroll containers can opt into the efficient path by calling a specific method, and the window will also attempt to detect scroll-only events automatically for containers that don't opt in.
...
impl Window {
    fn draw(&mut self, cx: &mut App) -> ArenaClearNeeded {
        ...
        if let Some(compositor) = self.compositor.as_ref() {
            // Fast path: commit to compositor
...
The question mark on `CommandEncoder` is the one piece we haven't fully verified. Modern wgpu (the `Far-Beyond-Pulsar/wgpu` fork at commit `fce5b80`, which reports as version 28.0.0 in Cargo.lock) may have made `CommandEncoder` `Send + Sync`. If it hasn't, the compositor creates the encoder and submits within the same function on the compositor thread — the encoder never crosses threads. The `CommandBuffer` resulting from `encoder.finish()` is `Send + Sync` and can be moved between threads.

In the worst case — if `CommandEncoder` is `!Send` — the design still works. The compositor creates the encoder, records commands, calls `encoder.finish()` to produce a `CommandBuffer`, and submits it to the queue. The encoder is dropped on the compositor thread. The command buffer (which is Send) is given to `queue.submit()`, which is also on the compositor thread. No thread crossing needed.

### The Atlas

`WgpuAtlas` uses `Mutex` internally, which makes it `Send + Sync`. The main thread calls atlas methods for slot allocation (from image loading, glyph rasterization). The compositor calls `atlas.before_frame()` for texture uploads. The mutex ensures mutual exclusion.

The one concern is lock contention. If the main thread allocates many atlas slots while the compositor is in `before_frame()`, the main thread blocks. We think this is acceptable because atlas allocation is rare — it happens when new content is first displayed, not every frame. During steady-state scrolling, no atlas allocation occurs, and `before_frame()` is a no-op.
...
Option B: Automatic detection in the window's event handler. The window tracks which views were notified during scroll event dispatch. If only scroll-related views (identified by a marker trait or type) were notified, the window treats it as a scroll-only event. No API change needed for scroll containers, but the detection heuristic may have false negatives (content + scroll in the same frame) or false positives (scroll event that also changes content in a non-scroll way).
...
### How Does Hit Testing Work with the Overscroll Buffer?

Hit testing in GPUI runs against the rendered frame's hitboxes, which are built during `draw_roots`. If we skip `draw_roots` for scroll-only events, the hitboxes would still reflect the old scroll position. A click at the new scroll position would hit-test incorrectly.

The solution is to apply the scroll offset to the hit testing results. The window tracks the scroll offset at the time of the last `draw_roots` and the current scroll offset. Hit testing against the old hitboxes is adjusted by the difference:
...
fn hit_test(&self, position: Point<Pixels>) -> HitTestResult {
    ...
    // Run hit test against rendered_frame.hitboxes using adjusted position
...
### How Does Focus Work During Scroll?
...
Animations that only affect position (like a smooth-scroll animation that interpolates scroll offset across multiple frames) should ideally use the overscroll fast path. The animation would update the scroll offset each frame without a commit, and the blit origin would smoothly track the animated offset. This requires the animation system to distinguish "position animation" from "property animation" and route them differently. We haven't designed this yet.

### What About Text Input?

Text input changes content — every keystroke modifies text, which changes the element tree (cursor position, new characters, potentially syntax highlighting). These always trigger `cx.notify()` and a full commit. The overscroll buffer offers no benefit here, but it doesn't hurt either — the commit re-renders the entire visible area into the overscroll buffer, and subsequent scroll events within the buffer are fast again.

This means the typical editing pattern — type a few characters, scroll to a new position, type more characters — alternates between full commits and fast scrolls. The user pays for the commits when they edit, but not when they scroll.
...
## What We'd Tell Ourselves if We Were Starting This Design Today

A few things that took us longer than expected to figure out:

**Measure before designing.** We spent two weeks on a theoretical analysis of thread safety that would have been faster if we'd just run `cargo doc --document-private-items` and checked the `Send` impls. The Scene types are all trivially Send, which we confirmed in a single afternoon once we actually looked. The wgpu types took longer because the documentation isn't always explicit about thread safety, but a quick look at the wgpu-core source confirmed that Queue, Device, Texture, and CommandBuffer are all Send+Sync on the version we're using.

**The channel capacity of 1 is non-negotiable.** We started with an unbounded channel because "what if the compositor needs to queue work?" The answer is: it doesn't. The compositor should never be ahead of the main thread by more than one frame. If it is, you're either overproducing scenes (build them less often) or under-consuming them (make the compositor faster). Dropping frames with `try_send` is the right behavior — it paces the main thread to the compositor's speed automatically.

**The atlas contention is real but manageable.** We were initially worried about the `WgpuAtlas` mutex being a bottleneck between the main thread and compositor. In practice, the main thread only acquires the atlas lock during new-image loading or glyph rasterization, which is rare during scrolling. The compositor acquires it during `before_frame()` on every commit. As long as the main thread doesn't need to load a new image in the middle of a scroll gesture, there's no contention. If it does, the main thread waits a few microseconds for the compositor to finish `before_frame()`, which is imperceptible.

**The fallback path is not optional.** When we started this design, we considered making the compositor required — if it fails to spawn, crash. We changed our minds after thinking about the test platform, the WebGPU target, and the debugging use case. Every code path in the compositor architecture has a synchronous fallback. The fallback is tested in CI. It's slower, but it works, and it means the compositor is an optimization rather than a dependency.
...
The design is split into three phases, each independently mergeable and independently testable.

### Phase 1: Compositor Thread (Computation Offload)

Add the compositor thread, pipeline texture, and blit path. Scene finishing, GPU uploads, and render pass encoding move to the compositor. The main thread does draw_roots, bookkeeping, and blit+present. The pipeline texture is viewport-sized — no overscroll yet.

**What changes:**
- NEW: `src/gpui/platform/cross/compositor.rs` — CompositorState, FrameCompositor
- MODIFY: `src/gpui/platform/cross/renderer.rs` — split draw(), add blit pipeline, extract pipeline ownership
- MODIFY: `src/gpui/platform/cross/window.rs` — create and use compositor
- MODIFY: `src/gpui/window/mod.rs` — use commit/try_present pattern

**What we verify:**
- All existing rendering tests pass (they use TestPlatform with no compositor)
- Frame profiling shows finish + upload + encoding offloaded to compositor thread
- No visual regression in any example

**What's not yet done:**
- Overscroll buffer (pipeline texture is viewport-sized)
- Scroll event interception (every scroll still triggers a full commit)

### Phase 2: Overscroll Buffer (Scroll Optimization)

Make the pipeline texture larger than the viewport. Track rendered area. Support tile extension and re-center.

**What changes:**
- MODIFY: `compositor.rs` — OverscrollBuffer, ExtendTiles and ReCenter job types
- MODIFY: `window/mod.rs` — scroll-aware blit origin, overscroll.covers() check
- MODIFY: `present()` path — try compositor, then overscroll, then fallback

**What we verify:**
- Scroll within overscroll buffer shows zero CPU rendering work
- Tile extension is a partial render (not full re-render)
- Re-center works correctly when scroll exceeds buffer
- present_framebuffer_only handles in-flight composites gracefully

**What's not yet done:**
- Scroll containers still call cx.notify() on every scroll event
- Performance gain is from the overscroll geometry, not from skipping draw_roots

### Phase 3: Scroll Event Interception (CPU Optimization)

Prevent scroll-only events from triggering `draw_roots`. Route them through the overscroll fast path.

**What changes:**
- NEW: Scroll-specific notification API (or auto-detection heuristic)
- MODIFY: UniformList, List, scrollable div to use new API
- MODIFY: window event handler to classify scroll events
- MODIFY: hit testing to apply scroll offset adjustment

**What we verify:**
- Scrolling uses the overscroll path (no draw_roots, no finish, no upload)
- cx.notify() still fires for actual content changes
- Hit testing works correctly with adjusted coordinates
- Focus handling works during scroll
- Nested scroll containers scroll independently
